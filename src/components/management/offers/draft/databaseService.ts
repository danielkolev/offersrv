...
import { generateDraftCode, hasMeaningfulContent } from './utils';
import { saveDraftToLocalStorage, getDraftFromLocalStorage, clearDraftFromLocalStorage } from './localStorageService';
import { DraftSaveOptions } from './types';

// Save draft to database for persistent storage
export const saveDraftToDatabase = async (userId: string, offer: Offer, options: DraftSaveOptions = {}): Promise<void> => {
  // Skip saving if there's no meaningful content and we're not forcing the save
  if (!options.force && !hasMeaningfulContent(offer)) {
