...
const ProductsForm = () => {
  ...
  const handleAddProduct = (isBundle: boolean = false) => {
    addProduct({
      ...
      unitPrice: 0,
      unit: defaultUnit,
      // Use the default unit from the hook
      isBundle: isBundle,
  ...
  const saveBundleProducts = () => {
    if (selectedProductId) {
      const totalBundlePrice = bundleProducts.reduce((sum, product) => sum + product.quantity * product.unitPrice, 0);
        ...
        {offer.products.map((product, index) => <div key={product.id} className="mb-6 relative rounded-lg border p-4 bg-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2 bg-gray-100">
              <div className="space-y-2">
                ...
                <Input id={`product-name-${index}`} value={product.name} onChange={e => updateProduct(product.id, {
              name: e.target.value
            })} placeholder={t.products.productName} className={!validateProductName(product.name) ? "border-destructive" : ""} />
                {!validateProductName(product.name) && <p className="text-sm text-destructive">{t.common.required}</p>}
              </div>
              
              {offer.details.showPartNumber && <div className="space-y-2">
                  <Label htmlFor={`product-part-${index}`}>{t.products.partNumber}</Label>
                  <Input id={`product-part-${index}`} value={product.partNumber || ''} onChange={e => updateProduct(product.id, {
              ...
              <div className={`space-y-2 ${offer.details.showPartNumber ? 'md:col-span-2' : 'md:col-span-1'}`}>
                ...
                <Textarea id={`product-desc-${index}`} value={product.description} onChange={e => updateProduct(product.id, {
              description: e.target.value
            })} placeholder={t.common.description} rows={2} />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor={`product-quantity-${index}`}>{t.products.quantity}</Label>
                <div className="flex gap-2">
                  <Input id={`product-quantity-${index}`} type="number" min="1" value={product.quantity} onChange={e => updateProduct(product.id, {
                quantity: parseInt(e.target.value) || 0
              })} />
                  
                  <Select value={product.unit || defaultUnit} onValueChange={value => updateProduct(product.id, {
                unit: value
              })}>
              ...
              <div className="space-y-2">
                ...
                <Input id={`product-price-${index}`} type="number" min="0" step="0.01" value={product.unitPrice} onChange={e => updateProduct(product.id, {
              unitPrice: parseFloat(e.target.value) || 0
            })} disabled={product.isBundle} />
              </div>
            </div>
            
            {product.isBundle && <div className="mt-4">
                <div className="flex justify-between items-center mb-2">
                  <Button variant="outline" size="sm" onClick={() => openBundleDialog(product.id)} className="flex-grow">
                    Manage Bundle Items ({product.bundledProducts?.length || 0} items)
                  </Button>
                  
                  <div className="flex items-center space-x-2 ml-4">
                    <Checkbox id={`show-bundle-prices-${index}`} checked={product.showBundledPrices} onCheckedChange={checked => updateProduct(product.id, {
                showBundledPrices: !!checked
              })} />
                    <Label htmlFor={`show-bundle-prices-${index}`} className="text-sm cursor-pointer">
                      Show bundled item prices
                    </Label>
                  </div>
                </div>
                
                {product.bundledProducts && product.bundledProducts.length > 0 && <div className="mt-2 border rounded-md p-2 bg-slate-50">
                    <p className="text-sm font-medium mb-1">Included items:</p>
                    <ul className="text-sm">
                      {product.bundledProducts.map(item => <li key={item.id} className="flex justify-between">
            ...
            <div className="text-right text-sm text-muted-foreground mt-1 bg-gray-100">
              {t.products.total}: {formatCurrency(product.quantity * product.unitPrice, language, currency)}
              {product.unit && product.unit !== 'none' && !product.isBundle && <span className="ml-1">({product.quantity} {getLocalizedUnitName(product.unit)})</span>}
            </div>
          </div>)}
        
        {offer.products.length === 0 && <div className="text-center py-8 text-muted-foreground">
      ...
      <Dialog open={bundleDialogOpen} onOpenChange={setBundleDialogOpen}>
        <DialogContent className="max-w-4xl">
          ...
          <div className="space-y-4 mt-4">
            <div className="border rounded-md p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                <div>
                  ...
                  <Input id="bundle-item-name" value={newBundleProduct.name} onChange={e => setNewBundleProduct({
                  ...newBundleProduct,
                  name: e.target.value
                })} />
                </div>
                ...
                <div>
                  ...
                  <Input id="bundle-item-qty" type="number" min="1" value={newBundleProduct.quantity} onChange={e => setNewBundleProduct({
                  ...newBundleProduct,
                ...
                <div>
                  ...
                  <Input id="bundle-item-price" type="number" min="0" step="0.01" value={newBundleProduct.unitPrice} onChange={e => setNewBundleProduct({
                  ...newBundleProduct,
                  unitPrice: parseFloat(e.target.value) || 0
                })} />
                </div>
            ...
            {bundleProducts.length > 0 ? <div className="border rounded-md p-4">
                <h3 className="font-medium mb-2">Bundle Items</h3>
                <div className="space-y-2">
                  {bundleProducts.map(item => <div key={item.id} className="flex items-center justify-between border-b pb-2">
                      <div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => removeBundleProduct(item.id)} className="h-8 w-8 text-muted-foreground hover:text-destructive">
                          <XIcon size={16} />
                        </Button>
                      </div>
                    </div>)}
                ...
                <div className="flex justify-between mt-4 pt-2 border-t">
                  ...
                  <span className="font-bold">
                    {formatCurrency(bundleProducts.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0), language, currency)}
                  </span>
                </div>
